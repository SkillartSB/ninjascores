...
(function () {
  ...
  function renderEmail() {
    ...
    } else if (pontGoogleNatif() && window.NS_GOOGLE_IOS_CLIENT_ID) {
      // App iOS 1.2+ : bouton maison qui ouvre la feuille de connexion Google du systeme.
      monterBoutonGoogleNatif(divGoogle, err);
    }
  ...
  function pontGoogleNatif() {
    try { return window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.ninjaGoogleSignIn; } catch (e) { return null; }
  }

  // « Continuer avec Google » dans l'app iOS (voir ContentView.swift, GoogleSignInBridge).
  // Charte Google : fond blanc, bordure #dadce0, logo G quatre couleurs.
  function monterBoutonGoogleNatif(conteneur, err) {
    var b = el('button', {
      type: 'button',
      style: { width: '100%', padding: '12px 16px', borderRadius: '14px', border: '1px solid #dadce0', background: '#fff', color: '#1f1f1f',
               fontSize: '15px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', fontFamily: 'inherit' }
    });
    b.innerHTML = '<svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true"><path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/><path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/><path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/><path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/></svg><span>Continuer avec Google</span>';
    b.onclick = function () {
      err.style.display = 'none';
      b.disabled = true; b.style.opacity = '0.7';
      var retablir = function () { b.disabled = false; b.style.opacity = ''; };
      window.NS_GOOGLE_NATIF = function (res) {
        window.NS_GOOGLE_NATIF = null;
        if (!res || res.erreur || !res.idToken) {
          retablir();
          if (res && res.erreur && res.erreur !== 'annule') { err.textContent = 'Connexion Google impossible. Réessaie.'; err.style.display = 'block'; }
          return;
        }
        SB.auth.signInWithIdToken({ provider: 'google', token: res.idToken, nonce: res.nonce }).then(function (r) {
          if (r.error) { retablir(); err.textContent = r.error.message; err.style.display = 'block'; return; }
          poursuivreApresApple(r.data.session);   // meme reprise que pour Apple (iOS : entree directe)
        }).catch(function () { retablir(); err.textContent = 'Erreur réseau, réessaie.'; err.style.display = 'block'; });
      };
      try { pontGoogleNatif().postMessage({ action: 'start', clientId: window.NS_GOOGLE_IOS_CLIENT_ID }); }
      catch (e) { window.NS_GOOGLE_NATIF = null; retablir(); }
    };
